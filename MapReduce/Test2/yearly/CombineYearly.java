...
public class CombineYearly extends Configured implements Tool {
		...
		if(line.length()>0)
		{
			...
			try {
	    		  	if ((nextLine = reader.readNext()) != null) 
				{
					...
					currentKey = new Text(compName+"\t"+dateSplit[2]);
					if(closeVal!="null")
						closeValNum = Double.parseDouble(closeVal);
					else
						closeValNum = -1;
					currentVal =new DoubleWritable(Double.parseDouble(closeVal));
  ...
  public static class Reduce extends Reducer<Text, DoubleWritable, Text, Text> {
	...
	double avg,tVal=0;
	if(headerFlag==0)
	{	headerFlag++;context.write(new Text("stock_name\tyear"),new Text("close"));}
