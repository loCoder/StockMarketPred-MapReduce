...
public class CombineMonthly extends Configured implements Tool {
	private static long start,end,end1;

  public static void main(String[] args) throws Exception {
				start = new Date().getTime();
    int res = ToolRunner.run(new CombineMonthly(), args);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
		...
		job1.setJarByClass(CombineMonthly.class);
		job1.setMapperClass(Map.class);
		job1.setReducerClass(Reduce.class);
		job1.setMapOutputKeyClass(Text.class);
		job1.setMapOutputValueClass(DoubleWritable.class);
		FileInputFormat.addInputPath(job1, new Path(args[0]));
		FileOutputFormat.setOutputPath(job1, new Path(args[1]+"Monthly"));
		...
		if(line.length()>0)
		{
			...
			try {
	    		  	if ((nextLine = reader.readNext()) != null) 
				{
					...
					double adj_closeNum = Double.parseDouble(adj_close);
					String[] dateSplit = date.split("-");
					currentKey = new Text(compName+"\t"+dateSplit[0]+"-"+dateSplit[1]);
					currentVal =new DoubleWritable(Double.parseDouble(adj_close));
  ...
  public static class Reduce extends Reducer<Text, DoubleWritable, Text, Text> {
	...
	double avg,tVal=0;
	if(headerFlag==0)
	{	headerFlag++;context.write(new Text("stock_name\tmonth"),new Text("adj_close"));}

	      Text currentKey,currentVal;
	...
	dayCount++;	
		...
		currentVal = new Text(avgD.toString());
		context.write(currentKey,currentVal);
