...
public class CombineYearly extends Configured implements Tool {
	private static long start,end;

  public static void main(String[] args) throws Exception {
				start = new Date().getTime();
    int res = ToolRunner.run(new CombineYearly(), args);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
		...
		job1.setJarByClass(CombineYearly.class);
		job1.setMapperClass(Map.class);
		job1.setReducerClass(Reduce.class);
		job1.setMapOutputKeyClass(Text.class);
		job1.setMapOutputValueClass(DoubleWritable.class);
		FileInputFormat.addInputPath(job1, new Path(args[0]));
		FileOutputFormat.setOutputPath(job1, new Path(args[1]+"Yearly"));
		...
		if(line.length()>0)
		{
			...
			try {
	    		  	if ((nextLine = reader.readNext()) != null) 
				{
					...
					currentKey = new Text(compName+"\t"+dateSplit[0]);
  ...
  public static class Reduce extends Reducer<Text, DoubleWritable, Text, Text> {
	...
	double avg,tVal=0;
	if(headerFlag==0)
	{	headerFlag++;context.write(new Text("stock_name\tyear"),new Text("adj_close"));}
